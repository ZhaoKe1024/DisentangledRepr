...
from dstgib_libs.basic_operations import chk_d
...
def conf_interval(data, axis=-1, conf_percent=90):
    ...
    interval = n * std
    # Return
    return {'avg': np.squeeze(avg, axis=axis), 'std': np.squeeze(std, axis=axis),
            'interval': np.squeeze(interval, axis=axis)}
