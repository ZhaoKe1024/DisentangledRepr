...
from dstgib_libs.dataloader_mnist import DataCycle
from dstgib_libs.logger import Logger, tfboard_add_multi_scalars
from dstgib_libs.basic_operations import fet_d, chk_ns, chk_d
from dstgib_libs.basic_metrics import TimersController, FreqCounter, StopWatch
        ...
        for key, logger in self._logs.items():
            ...
            for net_name, network in self._networks.items():
                logger.info_individually(
                    "Network[%s] total number of parameters : %.3f M. " % (net_name, network_param_m(network)),
                    to_screen)
